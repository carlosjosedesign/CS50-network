...
from .models import User, Post, Comment, Like, Following, Profile
from .forms import CreatePostForm, CreateCommentForm, CreateProfileForm
        ...
        if action == "post":
            ...
            if form.is_valid():
                ...
                try:
                    ...
                    return JsonResponse({"error": "Error to publish comment"}, status=404)
...
def edit_profile(request):
    ...
    if request.method == "POST":
        ...
        form = CreateProfileForm(request.POST, request.FILES, instance=request.user)

        if form.is_valid():
            # Get current user's profile
            new_profile = Profile.objects.get(user=request.user.id)
    ...
    if request.method == "POST":
        try:
            ...
            try:
                ...
                return JsonResponse({"massage": "error"}, status=404)
            else:
        ...
        return JsonResponse({"success": action }, status=201)
